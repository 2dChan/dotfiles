...
return {
  ...
  -- Main LSP configuration
  {
    ...
    config = function()
      ...
      -- Support for both Neovim 0.10 and 0.11 LSP client API
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          lsp_map('grn', vim.lsp.buf.rename, '[R]e[n]ame', 'n', buf)
          lsp_map('gra', vim.lsp.buf.code_action, '[G]oto Code [A]ction', { 'n', 'x' }, buf)
          lsp_map('grr', require('telescope.builtin').lsp_references, '[G]oto [R]eferences', 'n', buf)
          lsp_map('gri', require('telescope.builtin').lsp_implementations, '[G]oto [I]mplementation', 'n', buf)
          lsp_map('grd', require('telescope.builtin').lsp_definitions, '[G]oto [D]efinition', 'n', buf)
          lsp_map('grD', vim.lsp.buf.declaration, '[G]oto [D]eclaration', 'n', buf)
          lsp_map('gO', require('telescope.builtin').lsp_document_symbols, 'Open Document Symbols', 'n', buf)
          ...
          if client and client_supports_method(client, vim.lsp.protocol.Methods.textDocument_inlayHint, buf) then
            ...
            end, '[T]oggle Inlay [H]ints', 'n', buf)
          end
        end,
      })
